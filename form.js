...
// I have to replace all full stops with spaces, as Firebase will not accept a string with any of ".", "#", "$", "[", or "]"
const player_names_with_spaces = player_names_with_full_stops.replace(/\./g, ' ');

// this function parses the names data and fills the datalist
function load_player_names() {
    try {
        // here I'm parsing the names data (split by commas) and trims trailing whitespace from each name
        const captains = player_names_with_spaces.split(',').map(name => name.trim());

        // this gets the datalist element
        const datalist = document.getElementById('captains');

        // this fills the datalist with options
        captains.forEach(captain => {
            if (captain) { // this ensures that the captain name is not empty
...
// I'm calling the function to load captains as soon as the page loads
load_player_names();
...
if (yesPlayingRadio && noPlayingRadio && captaincyInput) {
    ...
    console.error("Playing or captain elements are missing in the DOM.");
}

// this shows or hides the time-watching input field based on radio button for watching
...
if (yesWatchRadio && noWatchRadio && time_watching_input) {
    ...
    console.error("Watching or time-watching elements are missing in the DOM.");
}
...
async function updateCharts() {
    // I'm using Promise.all to ensure all data is loaded before updating charts
    // I was having an issue before where the charts were rendering before all the data was loaded
    const [teamsSnapshot, teamsTotalSnapshot, watchingSnapshot, watchingTotalSnapshot, captainSnapshot, captainTotalSnapshot] = await Promise.all([
        database.ref('teams').once('value'),
        database.ref('teams-total').once('value'),
        database.ref('watching').once('value'),
        database.ref('watching-total').once('value'),
        database.ref('selected-captain').once('value'),
        database.ref('selected-captain-total').once('value'),
    ]);

    const teamsData = teamsSnapshot.val();
    const teamsTotalData = teamsTotalSnapshot.val();
    const watchingData = watchingSnapshot.val();
    const watchingTotalData = watchingTotalSnapshot.val();
    ...
    const totalWatching = watchingTotalData ? watchingTotalData.count : 0;
    ...
    updatePieChart(watchingData, totalWatching, createWatchingPieChart, 'watching_chart');
...
form.addEventListener('submit', (e) => {
    ...
    // I'm ensuring that the value of years-watching is not empty, otherwise this will still push data to firebase, but in the form of an entry directly in the node "years-watching"
    ...
    // the slice is called "count" and appears alongside the numbers of years watching
    ...
    // I'm also writing all the data to Firebase together as one entry, with a timestamp
    const newData = {
    ...
    database.ref('user_data').push().set(newData)
        .then(() => {
            console.log('user_data successfully written!');
            form.reset(); // this resets the form after successful submission
        })
        .catch((error) => {
            console.error('Error writing user_data:', error);
        }); 
    
    const pieChartsPlaceholder = document.getElementById("pie-chart-placeholder");
    
    if (pieChartsPlaceholder) {
        pieChartsPlaceholder.style.display = "none";
    } else {
        console.error("pie-chart-placeholder is missing in the DOM.");
    }

    // I found I had to set a delay to prevent retrieving the results before the user's data was fully submitted
    // that would cause the charts to render incorrectly
    setTimeout(() => updateCharts(), 500); 
});


// here I'm defining a function to produce a striped pattern given 2 colour
// this is to make the club support pie chart more readable
function StripedPattern(colour1, colour2) {
  const canvas = document.createElement('canvas');
  const ctx = canvas.getContext('2d');

  // I'm setting the canvas size
  canvas.width = 10;
  canvas.height = 10;

  // I'm using the first color as the background
  ctx.fillStyle = colour1;
  ctx.fillRect(0, 0, canvas.width, canvas.height);

  // then drawing vertical stripes with the second color
  ctx.strokeStyle = colour2;
  ctx.lineWidth = 10/3 ;
  ctx.beginPath();
  ctx.moveTo(5, 0); // the stripes start at the middle of the top edge
  ctx.lineTo(5, canvas.height); // the vertical line goes all the way to the bottom edge
  ctx.stroke();
...
function createTeamsPieChart(labels, dataValues, percentageLabels) {
    ...
    // I'm destroying the existing chart if it exists
    if (myTeamsChart) {
        myTeamsChart.destroy();
    }

    // here I'm defineing striped patterns for the chart
    const team_colours = [
        '#EF0107', 
        StripedPattern('#850322', '#88BBFF'), 
        StripedPattern('#BC0F14', '#000000'), 
        StripedPattern('#E30613', '#FFFFFF'), 
        StripedPattern('#005DAA', '#FFFFFF'), 
        '#034694',
        StripedPattern('#1B458F', '#C4122E'),
        '#274488',
        '#E5CBCB',
        '#3A64A3',
        '#0101E8',
        '#B21313',
        '#97C1E7',
        StripedPattern('#E80909', '#FFE500'),
        StripedPattern('#000000', '#FFFFFF'),
        '#E53233',
        StripedPattern('#ED1A3B', '#FFFFFF'),
        '#CCCCCC',
        StripedPattern('#7C2C3B', '#000000'),
        StripedPattern('#FDB913', '#231F20')
    ];

    // and making a new chart
    myTeamsChart = new Chart(ctx, {
        type: 'pie',
        data: {
            labels: labels.map((label, index) => `${label} (${percentageLabels[index]})`), // I'm appending the percentage to the label
            datasets: [{
                data: dataValues,
...
let myWatchingChart = null;
function createWatchingPieChart(labels, dataValues, percentageLabels) {
    const ctx = document.getElementById('watching_chart').getContext('2d');

    // I'm destroying the existing chart if it exists
    if (myWatchingChart) {
        myWatchingChart.destroy();
    }

    // and making a new chart
    myWatchingChart = new Chart(ctx, {
        type: 'pie',
        data: {
            labels: labels.map((label, index) => `${label} (${percentageLabels[index]})`), // I'm appending the percentage to the label
            datasets: [{
        ...
        options: { 
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: 'Do you watch the Premier League?'
                }
            }
        }
    });
